...
if [[ -z "${BUILD_ARCH}" && "${DO_CLEAN}" != "true" && "${DO_DISTCLEAN}" != "true" ]]; then
    BUILD_ARCH="armv7"
fi
...
ymd=$(date '+%Y%m%d')
mpd_version=$(PACKAGE="mpd"; load_package; echo "${PACKAGE_VERSION}")
if [[ -z "${INSTALL_PREFIX}" ]]; then
    if [[ ! -z "${mpd_version}" ]]; then
        INSTALL_PREFIX="/opt/mpd-${mpd_version}_${ymd}"
    else
        INSTALL_PREFIX="/opt/mpd_${ymd}"
    fi
fi
